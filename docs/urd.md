...
| Method | Endpoint | Mục đích | Input | Output
|--------|----------|----------|----------|----------|
| GET | `/api/work-orders/pending` | Lấy các lệnh chưa được xếp lịch để hiện ở thanh **Waiting List**. | NULL | Danh sách JobItem (Id, OrderNo, ProductName, Qty, DueDate, Status="Pool")| 
| GET | `/api/work-orders/{id}/route-details` | Xem quy trình và các máy có thể làm. (Khi Khi click vào Job thì gọi API) | id (WorkOrderId) | Object WorkOrderRouteDto chứa danh sách các Steps, mỗi step chứa danh sách các AvailableMachines (Id, Name, CycleTime)|
...
| POST | `/api/dispatching-board/assign` | Kéo đơn hàng thả vào máy. | { WorkOrderId, WorkCenterId, StepOrder, ScheduledStart } |  {ok: Success/Fail, error: “ …. ”} |
| DELETE | `/api/dispatching-board/{id}` | Hủy phân công, đưa task về lại danh sách chờ. |  |   |
| GET | `/api/dispatching-board?date=...` | Lấy các công việc đã được gán vào máy để hiển thị lên Board (thường dùng cho Gantt Chart hoặc hiển thị danh sách task dưới mỗi máy) | date (hoặc khoảng thời gian )| Danh sách các task đã gán (WorkOrderId, WorkCenterId, ScheduledStart, ScheduledEnd, Status). |
---
...
| PATCH |`/api/dispatching-board/{id}/status`| Cập nhật trạng thái thực tế của công việc. | { Status: "Processing" / "Completed" / "Hold" } | Cập nhật ActualStart (nếu bắt đầu) hoặc ActualEnd (nếu xong) |
...
| Method | Endpoint | Mục đích | Input | Output |
|--------|----------|----------| ----------|----------|
| GET | `/api/work-centers` | Xem danh sách máy và trạng thái (Xanh/Đỏ/Vàng). | NULL | Danh sách Machine (Id, Code, Name, Status [0: Offline, 1: Running, 2: Breakdown])  |
